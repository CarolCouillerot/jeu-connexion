...
class Plateau {
	...
	public Plateau(int taille, int k) {
		...
		genererPositionDebut('0',2);
		genererPositionDebut('1',2);
	}

	private void genererPositionDebut(char col, int k) {
		...
		while ( !aTester.isEmpty() ) {
			...
			for( Case v : voisin) {
				if( !dejaTeste[v.getX()][v.getY()] && !aTester.contains(v)) {
					if( v.getCouleur() != ' ' && v.getCouleur() != coul) {
	...
	public void afficher() {
		...
		for(int i = 0; i < taille_; ++i) {
			for(int j = 0; j < taille_; ++j) {
				if(plateau_[i][j].getCouleur() == '0')
					System.out.print(BG_LIGHTBLUE);
				else if(plateau_[i][j].getCouleur() == '1')
					System.out.print(BG_LIGHTGREY);
				System.out.print(plateau_[i][j].getCouleur()+BG_DEFAULT+"|");
			}
			System.out.println();
		}
		System.out.println("\n");
